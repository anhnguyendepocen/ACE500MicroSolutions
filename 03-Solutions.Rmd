...
b.  Note from a that the MRS is decreasing in $x$, so the function is quasi-concave.  
...
Notice that the marginal utility of $x$ is constant, so buying more of it doesn't contribute extra utility. Check out what happens as we increase income in the figure above. The equilibrium $y$ quantity increases, but the equilibrium $x$ quantity does not. If you don't have a good feel for it yet, print a couple indifference curves drawn with the [desmos calculator](https://www.desmos.com/calculator) and draw the tangent budget lines. You should see the same optimal $x$ value.   
